export const textDescriptions = {
    ...
    "ratchet": ["A ratchet consists of a rootkey, a remote ephemeral public key, and a local ephemeral keypair", 
    ...
    "x3dh": ["X3DH a procedure of running Diffie-Hellman key exchange 3 times and concatenating the result", 
            "A client performs X3DH to create sessions with one another", 
    ...
    "baseKey": ["The base key is the public key of a keypair that a client generates when running X3DH as initializer",
                "The client then include the base key in each prekey message.", 
                "The receiver of the prekey message uses the base key as input into its X3DH step"], 
    "prekeyBundle": [
    ...
    "identityKey": ["Identity key is a long term public/private keypair which represents the client's identity. ", 
                    "The Signal server stores each client's identity public key", 
                    "The client stores its own identity private key."], 
    "preKeyBundle": ["A PreKey bundle is a set of public keys generated by a client and sent to the Signal server", 
